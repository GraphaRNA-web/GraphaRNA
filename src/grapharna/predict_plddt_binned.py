...
def save_to_json(path, name, preds_for_rna):
    """Saves the pLDDT scores for a single RNA into OpenStructure JSON format"""
    local_lddt = {}
    
    for res_pos, score in enumerate(preds_for_rna):
        local_lddt[f"A.{res_pos + 1}"] = round(float(score), 4)
        
    global_lddt = round(float(np.mean(preds_for_rna)), 4)
    
    result_dict = {
        "global_lddt": global_lddt,
        "local_scores": local_lddt
    }
...
def main():
    ...
    # Output parameters
    parser.add_argument('--to_json', action='store_true', help='save results to json')
    ...
    with tempfile.TemporaryDirectory() as temp_base:
        ...
        plddt_head = pLDDTHead(input_dim=input_dim, num_bins=10).to(device)
        ...
        with torch.no_grad():
            for data, names, seqs in test_loader:
                ...
                try:
                    ...
                    print(f"Skipping batch containing {names} due to math error: {e}")
                    continue
                ...
                for i, num_res in enumerate(residue_counts):
                    ...
                    if args.to_json:
                        json_dir = osp.join(args.output_pdb_dir, "json_results")
                        os.makedirs(json_dir, exist_ok=True)
                        save_to_json(json_dir, rna_name.replace(".pdb", ".json"), preds_for_rna)
                    start_idx = end_idx
